...
from BLACK_MAMBA.utils import admin_cmd, humanbytes, progress, time_formatter
import subprocess
import patoolib
import tarfile
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".rar",
                caption="rarred By BLACK_MAMBA_BOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By BLACK_MAMBA_BOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                output,
                caption="TAR By BLACK_MAMBA_BOT",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
