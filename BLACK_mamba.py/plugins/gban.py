...
async def gspider(BLACK_MAMBA):
    ...
    me = await BLACK_MAMBA.client.get_me()
    await friday.edit(f"Global Ban Is Coming ! Wait And Watch You bitch😎🔥")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await BLACK_MAMBA.get_chat()
    a = b = 0
    if BLACK_MAMBA.is_private:
        user = BLACK_MAMBA.chat
        reason = BLACK_MAMBA.pattern_match.group(1)
    else:
        BLACK_MAMBA.chat.title
    try:
        user, reason = await get_full_user(BLACK_MAMBA)
    except:
        pass
    try:
        if not reason:
    ...
    if user:
        ...
        try:
            from BLACK_MAMBA.modules.sql_helper.gmute_sql import gmute
        except:
            pass
        try:
            await BLACK_MAMBA.client(BlockRequest(user))
        except:
            pass
        testBLACK_MAMBA = [
            d.entity.id
            for d in await BLACK_MAMBA.client.get_dialogs()
            if (d.is_group or d.is_channel)
        ]
        for i in testBLACK_MAMBA:
            try:
                await BLACK_MAMBA.client.edit_permissions(i, user, view_messages=False)
                a += 1
    ...
    try:
        ...
        pass
    POST(user=user.id, msg=BLACK_MAMBA.text[5:])
    await friday.edit(
...
async def gspider(BLACK_MAMBA):
    ...
    if user:
        ...
        try:
            from BLACK_MAMBA.modules.sql_helper.gmute_sql import ungmute
        except:
            pass
        try:
            await BLACK_MAMBA.client(UnblockRequest(user))
        except:
            pass
        testBLACK_MAMBA = [
            d.entity.id
        ...
        for i in testBLACK_MAMBA:
            try:
                await BLACK_MAMBA.client.edit_permissions(i, user, send_messages=True)
                a += 1
...
async def handler(rkG): 
   ...
   if rkG.user_joined or rkG.user_added:      
       try:       	
         from BLACK_MAMBA.modules.sql_helper.gmute_sql import is_gmuted
